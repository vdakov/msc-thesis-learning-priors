...
class BarDistribution(nn.Module):
    ...
    def map_to_bucket_idx(self, y):
        '''
        For N real outputs, says to which index they belong. At the borders, we set them to the edges respectively. 
        
        :param y: The output vectors to be mapped to bucket indices 
        '''
    ...
    def forward(self, logits, y): 
        ''' 
        Expects the logits of the predicted values and the target values of y to be tested against. 
        
        T - number of target samples 
        B - number of batches 
        
        logits: T X B x self.num_bars 
        y: T X B
        '''
    ...
    def mean(self, logits):
        '''
        Returns the predicted mean of the bar distribution, based on the logits - in a weighted average manner.
        '''
